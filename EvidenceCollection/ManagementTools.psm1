Import-Module ./AnalysisCommonTools.psm1 -Global -Force

[CmdLetBinding]
function New-AnalysisEnvironment {
    param(
        [Parameter(Mandatory = $true)]
        [string]$azureRegion,
        [Parameter(Mandatory = $false, ParameterSetName = "Inline Deployment Parameters")]
        [string]$caseNumber,
        [Parameter(Mandatory = $false, ParameterSetName = "Inline Deployment Parameters")]
        [string]$adminName,
        [Parameter(Mandatory = $false, ParameterSetName = "Inline Deployment Parameters")]
        ...
        [Parameter(Mandatory = $false, ParameterSetName = "Inline Deployment Parameters")]
        [string]$allowAdminToAccessKeyVault = $false,
        [Parameter(Mandatory = $false, ParameterSetName = "Inline Deployment Parameters")]
        [string[]]$analystIpAddresses,
        [Parameter(Mandatory = $false, ParameterSetName = "Inline Deployment Parameters")]
        [string[]]$readerIpAddresses,
        [Parameter(Mandatory = $false, ParameterSetName = "Specified Template Files")]
        [string]$templateFile = [System.IO.Path]::Combine('.', 'DeploymentTemplates', 'template.json'),
        [Parameter(Mandatory = $true, ParameterSetName = "Specified Template Files")]
...
if($VerbosePreference -ne 'SilentlyContinue') {
    ...
    if($MyInvocation.BoundParameters.ContainsKey("parametersFile")) {
        Write-Verbose "reading passed parameters file..."
        ...
        $templateParameterFileObject.parameters.Keys | 
            ForEach-Object { 
                $templateParametersObject.Add($_, $templateParameterFileObject.parameters[$_].value) 
            }
    } else {
        Write-Verbose "applying passed CmdLet parameters..."
        $MyInvocation.BoundParameters.Keys | 
            ForEach-Object { 
                $templateParametersObject.Add($_, $MyInvocation.BoundParameters[$_]) 
            }
    }

    # Id admin user object id was not passed as param, get from param file... or get from current user.
    if([string]::IsNullOrWhitespace($adminUserObjectId)) {
        $adminUserObjectId = $templateParametersObject['adminUserObjectId']
        if([string]::IsNullOrWhitespace($adminUserObjectId)) {
    ...
    $caseNumber = $templateParameterFileObject.parameters.caseNumber.value
